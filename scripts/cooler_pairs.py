...
def main():
    parser = argparse.ArgumentParser(
        description="""python matrix_storage_benchmark.py contacts_txt""")
    parser.add_argument('contacts_txt')
    parser.add_argument(
        '-s', '--square',
        type=int,
        default=256,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="The bin resolution in bp")

    args = vars(parser.parse_args())
    binsize = args['binsize']
    infilepath = args['contacts_txt']
    outfilepath = op.join(op.dirname(infilepath), 'GM12878-MboI.{}kb.cool'.format(binsize // 1000))

    # Build "index"
    t1 = time.time()
    chromsizes = cooler.read_chromsizes(
        'test/data/b37.chrom.sizes',
        name_patterns=[r'^[0-9]+$', r'^[XY]$', r'^MT$'])
    bins = cooler.binnify(chromsizes, binsize)
    chroms, lengths = zip(*six.iteritems(chromsizes))
    reader = cooler.io.TabixAggregator(infilepath, chromsizes, bins)
    with h5py.File(outfilepath, 'w') as h5:
       cooler.io.create(h5, chroms, lengths, bins, reader, assembly='b37') # metadata)
